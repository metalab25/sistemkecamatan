...
@section('content')
    @can('roles create')
        <button type="button" class="btn btn-primary btn-block mb-2 mb-sm-3 btn-add">Add Role</button>
    @endcan
    <div class="card mb-3">
...
@push('script')
    ...
    <script>
        ...
        $('#role-table').on('click', '.action', function() {
            ...
            $.ajax({
                method: 'get',
                url: `{{ url('settings/roles/') }}/${id}/edit`,
                success: function(res) {
                    $('#modalAction').find('.modal-dialog').html(res)
                    if ($(res).find('[name="permissions[]"]').length > 0) {
                        $('#modalAction').find('.modal-dialog').addClass('modal-lg')
                    }
                    modal.show()
                    store()
                }
