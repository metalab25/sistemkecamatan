...
@section('content')
    @can('permission create')
        <button type="button" class="btn btn-primary btn-block mb-2 mb-sm-3 btn-add">Add Permission</button>
    @endcan
    <div class="card mb-3">
...
@push('script')
    ...
    <script>
        ...
        $('.btn-add').on('click', function() {
            $.ajax({
                method: 'get',
                url: `{{ url('settings/permission/create') }}`,
            ...
            $('#formAction').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        modal.hide()
                        showToast(res.message || 'Permission created successfully!')
                    },
                    error: function(res) {
        ...
        $('#permission-table').on('click', '.action', function() {
            ...
            if (jenis == 'delete') {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            method: 'DELETE',
                            url: `{{ url('settings/permission/') }}/${id}`,
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf_token"]').attr('content')
                            },
                            success: function(res) {
            ...
            $.ajax({
                method: 'get',
                url: `{{ url('settings/permission/') }}/${id}/edit`,
                success: function(res) {
                    $('#modalAction').find('.modal-dialog').html(res)
                    modal.show()
                    store()
